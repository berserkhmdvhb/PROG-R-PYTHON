...
> Please do not use a separate document for answers.
...
judgments %>% 
  summarize(across(contains("moral_dil"),
                   list(aveg = ~ mean(.x, na.rm = TRUE)))) %>% 
  select(starts_with("moral_dil")) %>% 
...
```{r}
# Write your answer here
sample_ids <- filter(variant_significance, significance == "damaging") %>%
  select("variant") %>% unlist(use.names = FALSE)

sample_ids

```

> This is not a vector of sample ids
...
variant_significance_2<- variant_significance %>% 
  filter(significance == "damaging")

semi_join(variant_significance_2 ,variant_significance, by = "variant")
```
